...
void InitPipeline(){
    ...
    D3DCompileFromFile(GetAssetFullPath(L"copy.vs").c_str(),
                        nullptr,
...
void InitGraphics() {
    ...
    //create synchronization objects and wait until assest have been uploaded to the gpu
    ThrowIfFailed(g_pDev->CreateFence(0,D3D12_FENCE_FLAG_NONE,IID_PPV_ARGS(&g_pFence)));
    g_nFenceValue =1;

    //create event handle to use for frame synchronziation
