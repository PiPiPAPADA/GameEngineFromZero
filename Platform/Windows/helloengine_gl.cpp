...
bool InitializeOpenGL(HWND hwnd,int screenWidth,int screenHeight,float screenDepth,float screenNear,bool vsync){
    ...
    //query for a pixel format that fits the attribute we want
    result = wglChoosePixelFormatARB(g_deviceContex,attributeListInt,NULL,1,pixelFomat,&formatCount);
    if(result !=1){
        return false;
    }

    //if video card/display can handle out desired pixel format then we set an handler our desired pixel format then we set it as the current one
    result = SetPixelFormat(g_deviceContex,pixelFomat[0],&pixelFormatDescriptor);
    if(result != 1){
        return false;
    }
    ...
    g_renderingContext = wglCreateContextAttribsARB(g_deviceContex,0,attributeList);
    if(g_renderingContext == NULL){
        return false;
    }

    //set the rendering context to active
    result = wglMakeCurrent(g_deviceContex,g_renderingContext);
    if(result != 1){
        return false;
    }
    ...
    //set the polygon winding to front facing for left handle system
    glFrontFace(GL_CW);
    ...
    //initial world/model matrix to the identity matrix
    BuildIdentityMatrix(g_worldMatrix);
    //set field of view and screen aspect ratio
    fieldOfView = PI/4.0f;
    ...
    //store video card name in a class member variable so it can be retrieve
    ...
    //turn on or off the vertical sync denpending on the input bool value
    if(vsync){
...
bool InitializeExtensions(HWND hwnd){
    ...
    //set the temporary rendering context as current rendering context for the 
    error = wglMakeCurrent(deviceContext,renderContext);
    if(error!=1){
        return false;
    }

    //intialize the opengl extensions needed for this application
    result = LoadExtensionList();
    if(!result){
        return false;
    }

    //release the temporary rendering context now that extensions have been 
    wglMakeCurrent(NULL,NULL);
    wglDeleteContext(renderContext);
    renderContext = NULL;

    //release te device context for the window
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,LPTSTR lpCmdLine,int nCmdShow){
    ...
    RegisterClassEx(&wc);
    //create window
    hWnd = CreateWindowEx(WS_EX_APPWINDOW,_T("Hello Engine!"),_T("Hello Engine!"),
