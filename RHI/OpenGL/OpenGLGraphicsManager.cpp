...
bool My::OpenGLGraphicsManager::SetPerBatchShaderParameters(const char *paramName, float *param)
{
...
bool My::OpenGLGraphicsManager::SetPerFrameShaderParameters()
{
    unsigned int location;
...
void OpenGLGraphicsManager::CaculateCameraMatrix()
{
   auto& scene = g_pSceneManager->GetSceneForRendering();
    auto pCameraNode = scene.GetFirstCameraNode();
    if (pCameraNode) {
...
void My::OpenGLGraphicsManager::CaculateLights()
{
     auto& scene = g_pSceneManager->GetSceneForRendering();
