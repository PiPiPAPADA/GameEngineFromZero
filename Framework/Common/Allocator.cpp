...
void My::Allocator::Reset(size_t data_size, size_t page_size, size_t alignment)
{
    ...
    size_t minmal_size = (sizeof(BlockHeader)>m_szDataSize) ? sizeof(BlockHeader) : m_szDataSize;
#if defined(_DEBUG)
    //(alignment & (alignment-1)) gurantee alignment is 2^n
    assert(alignment>0 && ((alignment & (alignment-1)))==0);
#endif
    m_szBlockSize = ALIGN(minmal_size,alignment); 
    m_szAlignmentSize = m_szBlockSize-minmal_size;
...
{
    if(!m_pFreeList){
        PageHeader* pNewtPage = reinterpret_cast<PageHeader*>(new uint8_t[m_szPageSize]);
        ++m_nPages;
        m_nBlocks += m_nBlocksPerPage;
        m_nFreeBlocks += m_nBlocksPerPage;
#if defined(_DEBUG)
        FillFreePage(pNewtPage);
#endif
        if(m_pPageList){
            pNewtPage->pNext = m_pPageList;
        }
        m_pPageList = pNewtPage;
        
        BlockHeader* pBlock = pNewtPage->Blocks();
